...
const FILE_HEADER_VERSION_MAJOR = 1
const FILE_HEADER_VERSION_MINOR = 0
const FILE_HEADER_VERSION_PATCH = 0

var FILE_HEADER_MAGIC_BYTES = [...]byte{0x00, 0x6B, 0x76, 0x64, 0x62, 0x44, 0x41, 0x54}

const FILE_HEADER_SIZE = 24 // In bytes
...
func NewFileHeader(ts time.Time, offset uint32) *FileHeader {
	return &FileHeader{
		VersionMajor: FILE_HEADER_VERSION_MAJOR,
		VersionMinor: FILE_HEADER_VERSION_MINOR,
		VersionPatch: FILE_HEADER_VERSION_PATCH,
		Timestamp:    ts,
		Offset:       offset,
	}
}
...
// ReadFileHeader reads a data file header from the given file, it's assumed that the file pointer is at position 0, i.e. the beginning of the
...
// compatible
func ReadFileHeader(file afero.File) (*FileHeader, error) {
	var buf [FILE_HEADER_SIZE]byte
...
// WriteFileHeader writes the data file header to the file. Note: It's assumed that file pointer is at position 0 so that the header
...
func WriteFileHeader(fileHeader *FileHeader, file afero.File) error {
	var buf [24]byte

	// Copy magic bytes into buf
	copy(buf[:], FILE_HEADER_MAGIC_BYTES[:])

	buf[8] = FILE_HEADER_VERSION_MAJOR
	buf[9] = FILE_HEADER_VERSION_MINOR
	buf[10] = FILE_HEADER_VERSION_PATCH
